...
func (op opStoreSetKey) Do(t *testing.T, env interface{}) {
	err := op.Mgr.SetKey(*op.Ctx, op.Scheme, op.ID, op.Key)
	if op.ExpErr == "" {
...
func (op opStoreAddKey) Do(t *testing.T, env interface{}) {
	err := op.Mgr.AddKey(*op.Ctx, op.Scheme, op.ID, op.Key)
	if op.ExpErr == "" {
...
func (op opStoreRmKeys) Do(t *testing.T, env interface{}) {
	err := op.Mgr.RmKeys(*op.Ctx, op.Scheme, op.ID)
	if op.ExpErr == "" {
...
func (op opStoreGetKeys) Do(t *testing.T, _ interface{}) {
	recps, err := op.Mgr.GetKeys(*op.Ctx, op.Scheme, op.ID)
	if op.ExpErr == "" {
