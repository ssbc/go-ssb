		...
		for iter.Seek(prefix); iter.ValidForPrefix(prefix); iter.Next() {
			...
			err := it.Value(func(v []byte) error {
				if len(v) >= 1 && v[0] == '1' {
					...
					if err != nil {
						return fmt.Errorf("follows(%s): invalid ref entry in db for feed: %w", forRef.Sigil(), err)
					}
					fr, err := sr.Feed()
					if err != nil {
						return err
					...
					if err := fs.AddRef(fr); err != nil {
						return fmt.Errorf("follows(%s): couldn't add parsed ref feed: %w", forRef.Sigil(), err)
					}
				}
				return nil
			})
...
func (b *BadgerBuilder) recurseHops(walked *ssb.StrFeedSet, vis map[string]struct{}, who refs.FeedRef, depth int) error {
	...
	if err != nil {
		return fmt.Errorf("recurseHops(%d): couldnt estblish subfeeds for %s: %w", depth, who.Sigil(), err)
	}
	...
	if err != nil {
		return fmt.Errorf("recurseHops(%d): couldnt list subfeeds for list for %s: %w", depth, who.Sigil(), err)
	}
	...
	for j, subfeed := range subfeedList {
		...
		if err != nil {
			return fmt.Errorf("recurseHops(%d): add subfeed entry(%d) of %s failed: %w", depth, j, who.Sigil(), err)
		}
	...
	for i, followedByWho := range theirFollowList {
		...
		for j, subfeed := range subfeedList {
			err = walked.AddRef(subfeed)
			if err != nil {
				return fmt.Errorf("recurseHops(%d): add subfeed entry(%d) of %s failed: %w", depth, j, followedByWho.Sigil(), err)
			}
