...
func (ts *testSession) startGoBot(sbotOpts ...sbot.Option) {
	...
	ts.t.Logf("go-sbot: %s", sbot.KeyPair.ID().Sigil())
...
func (ts *testSession) startJSBotWithName(name, jsbefore, jsafter string) refs.FeedRef {
	...
	env := []string{
		"TEST_NAME=" + name,
		"TEST_BOB=" + ts.gobot.KeyPair.ID().Sigil(),
	...
	ts.t.Logf("JS %s:%d %s", name, jsBotCnt, jsBotRef.Sigil())
	return jsBotRef
}
...
func (ts *testSession) startJSBotAsServer(name, jsbefore, jsafter string) (refs.FeedRef, int) {
	...
	ts.t.Logf("JS %s: %s port: %d", name, srvRef.Sigil(), port)
	return srvRef, port
}
